...
Develop a strategy for deciding when to create a new design. If you create a Example new design every time you successfully research a new piece of technology,
...
same time. Try to leave room for at least one additional design, so that you colonized most or can create a new design quickly without first deleting an existing design. You all planets within could also create a new design for an existing class of ship, such as a colonizer, when the existing design has outlived its usefulness.


### Spend time in the Technology Browser learning about the ship technology you

think you’ll want to research. Take ship design into account when you plan your research strategies. If possible, design new ships only when you gain the technology you need to make a significant advance over older designs.
...
If the Copy Selected Design button is shadowed, you have reached the maximum of 16 concurrent ship designs. You must delete an existing design before you can design a new ship. based on this design. Only designs for which no ships exist may be edited. 4. Drag the components to be removed from the ship schematic to the
...
If you delete an existing design, all ships that use that design are destroyed, and their minerals are lost to the cosmos. If you wish to retrieve some of the minerals used in the ship, scrap all the ships using that design, then delete the
...
1. In the Ship and Starbase Designer, select Existing Designs. 2. Click on the dropdown list and select the design to delete.
...
To learn the total number of designs you’ve created: 1. In the Ship and Starbase Designer, select Existing Designs. 2. Click on the dropdown menu and count the number of designs. This
...
- Total ships of each design in a fleet: 32,000 - Different starbase designs per player: 10
...
Like Overthrusters, Maneuvering Jets give you an advantage in battle. One Maneuvering Jet gives the ship a speed bonus of one- hull type. If you enter into a battle with the ship, you learn which components Copy Selected are used in the design as well. To review enemy hull designs:

### War Mongers

A schematic of the ship displays. If you’ve encountered the ship only in passing, the schematic is empty. If you’ve met that devil in battle, all components are displayed. Right-click on each component to display further
