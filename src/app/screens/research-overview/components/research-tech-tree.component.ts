...
export class ResearchTechTreeComponent {
  ...
  readonly visibleLevels = computed(() => {
    ...
    return allLevels
      .map((level) => ({
        ...level,
        unlocks: level.unlocks.filter((name) => {
          const comp = this.techService.getComponentByName(name);
          if (comp) {
            return this.techService.isComponentAllowedByTraits(comp, player);
          }
          return true; // Hulls are always visible for now
        }),
      }))
      .filter((level) => level.level > 0 && level.unlocks.length > 0);
  });
